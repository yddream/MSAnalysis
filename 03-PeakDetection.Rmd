...
files <- list.files('Data', pattern = '(?i)mzxml$',full.names = TRUE,
        recursive = TRUE)
...
rawData <- readMSData(files = files, pdata = new("NAnnotatedDataFrame", pd),
               mode = "onDisk") 
pd
```
...
# xcms 3 new methods
cwp <- CentWaveParam(peakwidth = c(5, 50), noise = 1000, snthresh = 10)
...
### xcms中传统的峰检测方法
```{r xcms_old, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE, cache=TRUE}
require(xcms)
## Get the full path to the data files(mzxml)
files <- list.files('Data', pattern = '(?i)mzxml$',full.names = TRUE,
        recursive = TRUE)
xdata <- xcmsSet(files, method = 'centWave', peakwidth = c(5, 50), noise= 1000, snthr = 10)
...
```{r prepare, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
require(CAMERA)
...
```{r isotope, eval=FALSE, message=FALSE, warning=FALSE, dependson=c('prepare'), include=TRUE}
...
```{r addu, eval=FALSE, message=FALSE, warning=FALSE, dependson=c('isotope'), include=TRUE}
#Annotate adducts
...
```{r rules, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
...
```{r rules_mod, eval=FALSE, message=FALSE, warning=FALSE, dependson=c('prepare'), include=TRUE}
my.rules <- read.csv(files[4])
xac.addu <- findAdducts(xac.isotope, rules = my.rules, polarity="positive")
```
...
```{r output, eval=FALSE, message=FALSE, warning=FALSE, dependson=c('prepare'), include=TRUE}
